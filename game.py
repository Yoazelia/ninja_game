...
class Game:
    def __init__(self):
        ...
        self.pause_overlay.set_colorkey((128, 128, 128))
    ...
    def mainmenu(self):
        ...
        while True:
            self.screen.blit(
                pygame.transform.scale(self.assets["background"], (640, 480)), (0, 0)
            )
            self.clouds.update()
            self.clouds.render(self.screen)
            font = pygame.font.Font(None, 48)
            playbutton = pygame.Rect(220, 215, 200, 50)
            quitbutton = pygame.Rect(220, 315, 200, 50)
            Titletext = font.render("Ninja Game", True, (255, 0, 0))
            Playtext = font.render("Play", True, (255, 255, 255))
            Quittext = font.render("Quit", True, (255, 255, 255))
            pygame.draw.rect(self.screen, (135, 206, 235), playbutton)
            pygame.draw.rect(self.screen, (135, 206, 235), quitbutton)
            title_rect = Titletext.get_rect(centerx=640 // 2, y=115)
            self.screen.blit(Titletext, title_rect.topleft)
            self.screen.blit(
                Playtext, Playtext.get_rect(center=playbutton.center).topleft
            )  # Blit playtext onto the screen
            self.screen.blit(
                Quittext, Quittext.get_rect(center=quitbutton.center).topleft
            )
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    if playbutton.collidepoint(pygame.mouse.get_pos()):
                        self.running = True
                        self.run()  # Transition to the game scene
